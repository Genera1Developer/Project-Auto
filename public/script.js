document.addEventListener('DOMContentLoaded', function() {
    ...
    async function performLogin(username, password, captcha, errorMessage, encryptionStatus, encryptionAnimation) {
        try {
            const [encryptedData, salt, hmac] = await Promise.all([
            ...
            const encryptedHmac = await encryptHmac(hmac);
            ...
            const saltValue = sessionStorage.getItem('encryptionSalt');
    ...
    async function generateAndStoreHmacWebCrypto(data, salt) {
        try {
            ...
            showAlert('HMAC Gen & Storage Failed. Secure login disabled.', 'error');
