async function encryptAndSend(data, publicKey) {
  try {
    ...
    // Convert the ArrayBuffer to a Base64 string using btoa and Uint8Array
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const clearButton = document.getElementById('clearText'); // Added clear button
...
content: async function encryptAndSend(data, publicKey) {
  try {
    const enc = new TextEncoder();
    const encodedData = enc.encode(data);
