...
const encryptWithId = (data, userId) => {
    try {
        const combinedData = `${data}-${userId}`;
        const iv = crypto.randomBytes(ivLength);
        const { encryptedData, authTag } = encrypt(combinedData, iv);
        return {
            encryptedData: encryptedData.toString('base64'),
            iv: iv.toString('base64'),
            authTag: authTag.toString('base64')
...
const decryptWithId = (ivB64, authTagB64, encryptedDataB64, userId) => {
    if (!encryptedDataB64 || !ivB64 || !authTagB64) {
        return null;
    }
    ...
    try {
        ...
        if (decrypted) {
            const [data, id] = decrypted.split('-');
            if (parseInt(id) === userId) {
                return data;
            } else {
                console.warn("User ID mismatch during decryption.");
                return null;
            }
        }
        return null;
