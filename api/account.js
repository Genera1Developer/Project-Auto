...
exports.createUser = async (username, password, callback) => {
    ...
    try {
        ...
        db.run(`INSERT INTO users (username, password, salt, password_version, encryption_iv, auth_tag) VALUES (?, ?, ?, ?, ?, ?)`, [Buffer.from(usernameEncryption.encryptedData, 'hex'), Buffer.from(passwordEncryption.encryptedData, 'hex'), Buffer.from(saltEncryption.encryptedData, 'hex'), PBKDF2_ITERATIONS, Buffer.from(usernameEncryption.iv, 'hex'), Buffer.from(usernameEncryption.authTag, 'hex')], function(err) {
            if (err) {
                console.error(err.message);
                return callback(err);
            }
...
exports.verifyUser = (username, password, callback) => {
    try {
        ...
        db.get(`SELECT id, username, password, salt, password_version, encryption_iv, auth_tag FROM users WHERE username = ?`, [Buffer.from(usernameEncryption.encryptedData, 'hex')], async (err, row) => {
            if (err) {
                console.error(err.message);
                return callback(err);
            }
            ...
            try {
                const decryptedUsername = decryptData(row.username.toString('hex'), row.encryption_iv.toString('hex'), row.auth_tag.toString('hex'));
                if (!decryptedUsername) {
                    return callback(new Error("Username decryption failed"));
                }

                const decryptedSalt = decryptData(row.salt.toString('hex'), row.encryption_iv.toString('hex'), row.auth_tag.toString('hex'));
                if (!decryptedSalt) {
                    return callback(new Error("Salt decryption failed"));
                }

                const decryptedPassword = decryptData(row.password.toString('hex'), row.encryption_iv.toString('hex'), row.auth_tag.toString('hex'));
                if (!decryptedPassword) {
                    return callback(new Error("Password decryption failed"));
                }
